...
model = dict(
    ...
    train_cfg=dict(
        assigner=dict(
            ...
            match_costs=[
                dict(type='FocalLossCost', weight=cls_loss_coef),  # from 6.0 to 2.0
                dict(type='BBox1dL1Cost', weight=seg_loss_coef, box_format='xywh'),
                dict(type='IoU1dCost', iou_mode='giou', weight=iou_loss_coef)])),  # from iou to giou
