...
from my_modules.loss import PositionFocalLossCost
from mmdet.structures.bbox import bbox_overlaps


@TASK_UTILS.register_module()
class CustomHungarianAssigner(BaseAssigner):
    ...
    def assign(self,
        ...
        for match_cost in self.match_costs:
            if isinstance(match_cost, PositionFocalLossCost):
                bboxes = pred_instances.bboxes.detach()
                bboxes[:, 1] = 0.1
                bboxes[:, 3] = 0.9
                giou = bbox_overlaps(bboxes, gt_instances.bboxes, mode='giou').clamp(min=1e-6)
                cost = match_cost(
