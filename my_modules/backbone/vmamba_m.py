...
from my_modules.backbone.decorator import merge_crops_and_clips
...
class VisionMamba(BaseModule):
    ...
    def forward_features(self, x, inference_params=None):
        ...
        hidden_states = hidden_states[:, 1:, :] + cls_tokens * 0
        hidden_states = rearrange(hidden_states, "b (t h w) m -> b m t h w", t=T, h=H, w=W)
...
if __name__ == '__main__':
    ...
    input = torch.rand(1, 3, num_frames, img_size, img_size).cuda()
    vmamba_m = videomamba_middle(num_frames=num_frames).cuda()
    flops = count_and_print(vmamba_m, input)
