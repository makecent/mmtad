...
class Pseudo4DRegLinear(nn.Linear):
    ...
    However, it uses inside a Linear(in_dim, 2) to regress the (cx, w)/(x1, x2) and the y-axis information
...
class Pseudo2DLinear(nn.Linear):
    ...
    However, it uses inside a Linear(in_dim, 1) to regress the (x)/(delta-x) and the y-axis information
    is constant zeros. That why it's called "pseudo2D".
